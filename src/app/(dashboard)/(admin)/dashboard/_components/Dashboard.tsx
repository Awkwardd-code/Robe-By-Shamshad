...
const Products: React.FC = () => {
    ...
    // Slug generation function for fashion products
    const generateSlug = (name: string): string => {
        const normalSlug = name
            .toLowerCase()
            .replace(/[^a-z0-9]+/g, '-')
            .replace(/(^-|-$)/g, '');

        if (!normalSlug || normalSlug.length === 0 || /^[-\s]*$/.test(normalSlug)) {
            const characters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789';
            let randomString = '';
            for (let i = 0; i < 8; i++) {
                randomString += characters.charAt(Math.floor(Math.random() * characters.length));
            }
            return `robe_by_shamshad_${randomString}`;
        }
    ...
    const handleAddSubmit = async (e: React.FormEvent) => {
        ...
        const productWithSlug = {
            ...
            sku: newProduct.sku || generateSlug(newProduct.name),
    ...
    const handleUpdateSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const productWithSlug = {
                ...
                sku: selectedProduct.sku || generateSlug(selectedProduct.name),
                                            ...
                                            <input
                                                ...
                                                onChange={(e) => {
                                                    ...
                                                    setNewProduct({
                                                        ...newProduct,
                                                        name: newName,
                                                        sku: !newProduct.sku || newProduct.sku === generateSlug(newProduct.name)
                                                ...
                                                <input
                                                    ...
                                                    onChange={(e) => {
                                                        ...
                                                        setSelectedProduct({
                                                            ...selectedProduct,
                                                            name: newName,
                                                            sku: !selectedProduct.sku || selectedProduct.sku === generateSlug(selectedProduct.name)
                                                                ? generateSlug(newName)
                                                                : selectedProduct.sku
                                                        });
                                                    }}
